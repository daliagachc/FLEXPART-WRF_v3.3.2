...
subroutine readwind_nests(indj, n, uuhn, vvhn, wwhn, divhn)
    ...
    do l = 1, numbnests
        ...
        if (ierr == -1) then
            ...
            iduma = nint(duma)
            if (iduma /= wftime(indj)) goto 1100
        end if
        ...
        !          stop
        !      end if
        !      if (abs(p_top_wrf - duma) > toler*abs(p_top_wrf)) then
        ...
        do j = 0, nyn(l) - 1
            do i = 0, nxn(l) - 1
                if (abs(ylat2dn(i, j, l) - dumarray_pp(i, j, 1)) >  &
        ...
        do j = 0, nyn(l) - 1
            do i = 0, nxn(l) - 1
                if (abs(xlon2dn(i, j, l) - dumarray_pp(i, j, 1)) >  &
        ...
        if (wind_option<=0) varname = 'U'
        if (wind_option==1) varname = 'AVGFLX_RUM'
        if (wind_option==2) varname = 'U'
        ...
        if (time_option==0) then
            ...
            if (ierr /= 0) then
                ...
                if (wind_option<=0) print*, 'you asked snapshot winds'
                if (wind_option==1) print*, 'you asked mean winds'
        ...
        if (wind_option<=0) varname = 'V'
        if (wind_option==1) varname = 'AVGFLX_RVM'
        if (wind_option==2) varname = 'V'
        ...
        !     varname = 'W'
        if (wind_option<=0) varname = 'W'
        if (wind_option==1) varname = 'AVGFLX_WWM'
        if (wind_option==2) varname = 'WW'
        ...
        if (turb_option == turb_option_tke .or. &
                turb_option == turb_option_mytke) then
            !-
            ! TKE - read TKE
            !   the wrf output file contains (nuvz-add_sfc_level) levels
            !   read the data into k=kbgn,nuvz
        ...
        !todo: uncommented by Diego so that we avoid p0<p1 in line 1419?
        ...
        if (lu_option==1) then  ! Read land-use from WRF?
            varname = 'LU_INDEX'
            call read_ncwrfout_1realfield(ierr, idiagaa, fnamenc, &
            ...
            do k = 1, numclass  ! Re-initialize xlandusen
                ...
                enddo
            enddo
            if (num_land_cat==24) then ! Assume USGS landuse data
                do j = 0, nyn(l) - 1
                    do i = 0, nxn(l) - 1
                    ...
                    enddo
                enddo
            elseif (num_land_cat==20) then ! Assume MODIS data
                do j = 0, nyn(l) - 1
                    do i = 0, nxn(l) - 1
                        k = nint(landuse_wrf(i, j))  ! Safely convert element to integer (nearest)
        ...
        if (ierr /= 0) then
            ...
            !         do 1502 j=0,nyn(l)-1
            !         do 1502 i=0,nxn(l)-1
            !            if(sshfn(i,j,1,n,l)>200.) sshfn(i,j,1,n,l)=200.
            !            if(sshfn(i,j,1,n,l)<-400.) sshfn(i,j,1,n,l)=-400.
            !1502     continue
        end if
        ...
        if(sfc_option == sfc_option_wrf) then
            ! pblh
            varname = 'PBLH'
            call read_ncwrfout_1realfield(ierr, idiagaa, fnamenc, &
        ...
        ! CALCULATE SURFSTR
        if(sfc_option == sfc_option_diagnosed) then
            do j = 0, nyn(l) - 1
                do i = 0, nxn(l) - 1
        ...
        if (add_sfc_level == 1) then
            do  j = 0, nyn(l) - 1
                do  i = 0, nxn(l) - 1
                    ...
                    ! pressure at 2 m AGL
                    !todo:Diego: This makes no sense? might cause: pphn(i,j,1)<(pphn(i,j,2)
                    !actually, as pointed out by Victoria, this wont produce |^
                    ...
                    if (hmixn(i, j, 1, n, l)<hmixmin) then
                        hmixn(i, j, 1, n, l) = hmixmin
                    end if
