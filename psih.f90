...
! hmjb: declared function type:
real function psih (z,lin)
  !*****************************************************************************
  !                                                                            *
  ...
  !                                                                            *
  !     2015-05-07 A. Dingwell: Modified code for better precision and clarity *
  !                   as suggested by H.M.J. Barbosa                           *
  !*****************************************************************************
  ...
  !hmjb fix function and in/out declarations
  real, intent(in) :: z,lin !hmjb: added intent
  real*8 :: x,zeta,l !hmjb
  ...
  if ((log10(z)-log10(abs(l))).lt.log10(eps)) then
    ...
    zeta=z/l
    if (zeta.gt.0.) then
      !hmjb: changed exp() to double precision dexp() to avoid underflows
      psih = - (1.+0.667*a*zeta)**(1.5) - b*(zeta-c/d)*dexp(-d*zeta) &
        - b*c/d + 1.
    else
      x=(1.-16.*zeta)**(.25)
      psih=2.*log((1.+x*x)/2.)
